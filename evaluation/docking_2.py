...
def sdf_to_pdbqt(sdf_file, pdbqt_outfile, mol_id):
    os.popen(f'/mnt/rna01/lzy/miniconda3/envs/mol/bin/obabel {sdf_file} -O {pdbqt_outfile} ').read()
    return pdbqt_outfile
...
def calculate_qvina2_score(receptor_file, sdf_file, out_dir, size=20,
    ...
    if receptor_file.suffix == '.pdb':
        ...
        os.popen(f'/mnt/rna01/lzy/miniconda3/envs/adt/bin/prepare_receptor4.py -r {receptor_file} -O {receptor_pdbqt_file}')
        # os.popen(f'./evaluation/prepare_receptor4.py -r {receptor_file} -O {receptor_pdbqt_file}')
    else:
    ...
    for i, mol in enumerate(suppl):  # sdf file may contain several ligands
        ...
        if out_pdbqt_file.exists() and not out_sdf_file.exists():
            os.popen(f'/mnt/rna01/lzy/miniconda3/envs/mol/bin/obabel {out_pdbqt_file} -O {out_sdf_file}').read()
        ...
        if out_sdf_file.exists() and out_sdf_file.stat().st_size != 0:
            ...
            # run QuickVina 2
            out = os.popen(
                f'/mnt/rna01/lzy/software/qvina2.1 --receptor {receptor_pdbqt_file} '
