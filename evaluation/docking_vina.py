...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_filename, protein_root='./data/crossdocked', **kwargs):
        # load original pdb
        protein_fn = os.path.join(
            os.path.dirname(ligand_filename), 
            os.path.basename(protein_filename)  # PDBId_Chain_rec.pdb
        )
        protein_path = os.path.join(protein_root, protein_fn)
        return cls(protein_path, ligand_rdmol, **kwargs)
    ...
    def extract_atoms_from_pdbqt(self, content, skip_hydrogens=True):
        import numpy as np
        atoms = []
        for line in content.splitlines():
            if line.startswith("ATOM") or line.startswith("HETATM"):
                atom_name = line[12:16].strip()
                if skip_hydrogens and atom_name.startswith("H"):
                    continue
                x = float(line[30:38])
                y = float(line[38:46])
                z = float(line[46:54])
    ...
    def run(self, mode='dock', exhaustiveness=8, index=0, **kwargs):
        ...
        if mode == 'dock':
            ...
            ligand_coords = self.extract_atoms_from_pdbqt(ligand_content)
            pose_coords = self.extract_atoms_from_pdbqt(pose)
            if ligand_coords.shape != pose_coords.shape:
                raise ValueError("Coordinate arrays must have the same shape")
            rmsd = kabsch_rmsd(ligand_coords, pose_coords)

            ligand_dir = os.path.dirname(self.receptor_path)
            ligand_basename = os.path.splitext(os.path.basename(self.receptor_path))[0]
            pose_output_path = os.path.join(ligand_dir, f"{ligand_basename}_{index}_docked.pdbqt")
            with open(pose_output_path, 'w') as f:
                f.write(pose)

            return [{'affinity': score, 'pose': pose, "rmsd": rmsd}]
...
def kabsch_rmsd(P, Q):
    """
    P and Q are Nx3 numpy arrays (coordinates of the same atoms)
    Returns RMSD after optimal alignment (Kabsch algorithm)
    """
