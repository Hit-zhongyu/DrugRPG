...
class EGNN_Sparse(MessagePassing):
    ...
    def forward(self, z: Tensor, edge_index: Adj = None,
...
class EGNN_Sparse_Network(nn.Module):
    ...
    def forward(self, h, x, edge_index, batch, edge_attr=None, linker_mask=None,
        ...
        # CA
        # position = self._get_position(batch)
        # h = self.atom_emb(z) + self.pos_emb(position)
        # h = self.ligandemb(h)
        # h = self.LayerNorm(h)
