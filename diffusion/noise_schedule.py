...
class NoiseScheduleVP:
    def __init__(self,
                 ...
                 sigmoid_scale_start=0.9999,  
                 sigmoid_scale_end=0.0001,    
                 sigmoid_width=3.0,
                 gamma=4.0,
                 dtype=torch.float32):
        """
        ...
        if schedule not in ['discrete', 'linear', 'cosine', 'discrete_poly', 'sigmoid', 'slowed_cosine', 'slowed_cosine2']:
            raise ValueError(
                "Unsupported noise schedule {}. The schedule needs to be 'discrete' or 'linear' or 'cosine' or 'sigmoid'".format(
                    schedule))
        ...
        if 'discrete' in schedule:
            ...
            k = torch.tensor(sigmoid_width)
            self.end = sigmoid_scale_end
            self.start = sigmoid_scale_start
            s = (self.end - self.start)/(torch.sigmoid(-k) - torch.sigmoid(k))
            b = 0.5 * (self.end + self.start - s)
            self.sigmoid_s = s
            self.sigmoid_b = b
            self.sigmoid_k = k
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if 'discrete' in self.schedule:
            ...
            x = 2.0 * (t / self.T) - 1.0 
            log_alpha = torch.log(
                self.sigmoid_s * torch.sigmoid(-self.sigmoid_k * x) + self.sigmoid_b
            )
            return log_alpha
        elif self.schedule == "slowed_cosine":
            alpha_01 = 1.0 - t**self.gamma
            log_alpha = self.end + (self.start - self.end) * alpha_01
            alpha_clamped = torch.clamp(log_alpha, min=1e-30)
            log_alpha = torch.log(alpha_clamped)
            return log_alpha
        
        elif self.schedule == "slowed_cosine2":
            alpha_01 = 1.0 - t**self.gamma
            log_alpha = self.end + (self.start - self.end) * alpha_01
            log_alpha = torch.where(t > 0.999, torch.tensor(0.00, device=t.device, dtype=t.dtype), log_alpha)
            alpha_clamped = torch.clamp(log_alpha, min=1e-30)
            log_alpha = torch.log(alpha_clamped)
            return log_alpha
    ...
    def tr_sigma(self, t):
        tr_sigma = self.beta_0 ** (1-t) * self.beta_1 ** t
        return torch.log(1 + 3*tr_sigma) + 6
