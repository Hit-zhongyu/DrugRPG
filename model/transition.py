...
from .utils import categorical_kl, extract, index_to_log_onehot, log_1_min_a, log_add_exp, log_categorical, log_sample_categorical, to_torch_const
...
class ContigousTransition(nn.Module):
    ...
    def add_noise(self, x, time_step, batch, torch_seed):
        ...
        noise.normal_(generator=torch_seed)
    ...
    def sample_init(self, shape):
        ligand_pos = torch.zeros(shape, device=self.betas.device)
        return ligand_pos.normal_()
...
class AtomTransition(nn.Module):
    def __init__(self, betas, num_classes):
        super().__init__()
        self.num_classes = num_classes

        log_alphas_v = np.log(betas)
    ...
    def q_vt_pred(self, log_v0, t, batch):
        # compute q(vt | v0)
        ndim = log_v0.ndim
        log_alpha_bar_t = extract(self.log_alphas_cumprod_v, t, batch, ndim=ndim)
        log_1_min_alpha_bar = extract(self.log_one_minus_alphas_cumprod_v, t, batch, ndim=ndim)
    ...
    def q_v_pred_one_timestep(self, log_vt_1, t, batch):
        # q(vt | vt-1)
        ndim = log_vt_1.ndim
        log_alpha_t = extract(self.log_alphas_v, t, batch, ndim=ndim)
        log_1_min_alpha_t = extract(self.log_one_minus_alphas_v, t, batch, ndim=ndim)
    ...
    def sample_init(self, n):
        init_log_atom_vt = torch.zeros(n, self.num_classes).to(self.log_alphas_v.device)
